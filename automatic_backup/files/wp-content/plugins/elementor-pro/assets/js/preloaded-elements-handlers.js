/*! elementor-pro - v3.8.2 - 23-11-2022 */
"use strict";
...
var _default = elementorModules.frontend.handlers.Base.extend({
  ...
  getDefaultElements() {
    ...
    elements.$stickyMenu = this.$element.closest(selectors.stickyMenu);
    return elements;
  },
  ...
  // Calculates the mobile menu height, in order to prevent the menu from overflowing the viewport for sticky menus, and remove menu height limitation for non sticky menus.
  reassignMobileMenuHeight() {
    if (this.$element.hasClass('elementor-sticky')) {
      const wpAdminBarHeight = jQuery('#wpadminbar')?.outerHeight() ?? 0;
      const menuToggleHeight = this.elements.$menuToggle?.outerHeight() ?? 0;
      const requiredMenuHeight = elementorFrontend.elements.$window.height() - menuToggleHeight - wpAdminBarHeight;
      this.elements.$dropdownMenu.css('max-height', requiredMenuHeight + 'px');
    } else {
      this.elements.$dropdownMenu.css('max-height', 'initial');
    }
  }
...
class ProductAddToCart extends _base.default {
  ...
  bindEvents() {
    ...
    this.elements.$quantityInput.on('change', () => {
      this.updateAddToCartButtonQuntity();
    });
  }
