...
class Product_Add_To_Cart extends Base_Widget {
	...
	private function render_loop_add_to_cart( $product ) {
		?>
		<form class="cart e-loop-add-to-cart-form">
			...
			if ( 'simple' === $product->get_type() ) {
				...
				if ( 'yes' === $this->get_settings_for_display( 'show_quantity' ) ) {
	...
	private function is_product_single() {
		return ( ! $this->is_loop_item_template_edit() && is_product() );
	}

	private function is_loop_item_template_edit() {
		return ( Plugin::elementor()->editor->is_edit_mode() && 'loop-item' === Plugin::elementor()->documents->get_current()->get_type() );
	}

	public function should_add_container() {
		$settings = $this->get_settings_for_display();
		return in_array( $settings['layout'], [ 'auto', 'stacked' ], true );
	}
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .added_to_cart' => 'margin-inline-start: {{SIZE}}{{UNIT}}',
				],
			]
		);
